...
namespace $TJ::$ResPkgUtil::$DZUtil::$Packer {
	...
	namespace $UnPack {
		Void expUnPackDflt(Byte const * const & data, SzI const & sz, Path const & mainDir) { UnPack(data, sz, Path(mainDir).add($DfltPath::kStructInfoFile), Path(mainDir).add($DfltPath::kResDir)); return; }
